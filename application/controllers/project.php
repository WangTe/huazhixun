...
{	
	...
	public function type() 
	{
		...
		$this->load->view('project_list.php');
		$this->load->view('info_right.php');		
		$this->load->view('footer.php');
	}
	...
	function search() 
	{
		...
		$data['projects'] = $this->project_m->get_search($keyword, $per_page, $per_page * ($p - 1));
		$total_rows = $this->project_m->get_num(0, $keyword);
	...
	private function _page_init_search($per_page, $total_rows)
	{
		...
		$config['per_page'] = $per_page;
		$config['base_url'] = base_url('project/search/?keyword=' . $keyword);
		$config['num_links'] = 20;
		$config['query_string_segment'] = 'p';
		$config['first_link'] = '首页';
		$config['last_link'] = '末页';
